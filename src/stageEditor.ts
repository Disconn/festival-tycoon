...
export function mountStageEditor(getGame:()=>GameState,toast:(s:string,error?:boolean)=>void){
  const panel=document.createElement('section');panel.className='stage-editor';panel.hidden=true;panel.setAttribute('role','dialog');panel.setAttribute('aria-modal','true');panel.setAttribute('aria-label','Bühnenwerkstatt')
  panel.innerHTML=`<header><div><small>PIXEL RIG LAB</small><h2>Deine Bühne. Deine Show.</h2></div><button data-close aria-label="Bühneneditor schließen">×</button></header>
